...
const getInitCells = (): Cell[] => {
	return [
		{ id: "0", type: "text", content: "# **JavaScript scratch pad**" },
		{
			id: "1",
			type: "text",
			content: "Just import something... and modules it will automatically be downloaded and bundled.",
		},
		{
			id: "2",
			type: "code",
...
export const fetchCells = () => {
	return async (dispatch: Dispatch<Action>) => {
		...
		try {
			...
			if (data !== null && data.length > 0) dispatch({ type: ActionType.FETCH_CELLS_COMPLETE, payload: data });
			else {
